...
import dev.selfmadesystem.dmm.installer.step.installing.InstallStep
import dev.selfmadesystem.dmm.installer.step.patching.AddVendettaStep
import dev.selfmadesystem.dmm.installer.step.patching.PatchManifestsStep
import dev.selfmadesystem.dmm.installer.step.patching.PresignApksStep
import dev.selfmadesystem.dmm.installer.step.patching.ReplaceIconStep
import dev.selfmadesystem.dmm.installer.util.LogEntry
import dev.selfmadesystem.dmm.installer.util.Logger
    ...
    val logger = Logger("StepRunner").also { logger ->
        debugInfo.split("\n").forEach {
            logger.logs += LogEntry(
                it,
                LogEntry.Level.INFO
            ) // Add debug information to logs but don't print to logcat
        }
    }
